...
def main(argv):
    ...
    def add_common_args(parser):
        ...
        output_args.add_argument(
            "-i", "--in_place", action="store_true", help="Alters config file in place"
        )
        output_args.add_argument(
            "-o",
            "--out",
            ...
            metavar="FILE",
            help="Where to write modified config file",
            default=sys.stdout,
        )
        parser.add_argument(
            "config_file",
...
def update(args):
    ...
    new_config = config.new(
        project_fp=old_config.get("all", {}).get("version", ""), template=args.template
    )
