...
def config():
    ...
    if not shutil.which("mamba"):
        sys.exit("Sunbeam needs mamba to be installed to run tests `conda install -c conda-forge mamba`. If you want to run sunbeam (not the tests) without mamba you can use `sunbeam run --profile /path/to/project/ --conda-frontend=conda`.")
